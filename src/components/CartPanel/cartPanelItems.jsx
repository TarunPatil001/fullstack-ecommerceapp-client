      ...
      <div className="cartItem w-full flex items-start gap-3 p-2">
        ...
        <div className="cartInfo w-[80%] pr-5 relative">
          ...
          <div className='flex items-center text-[12px] gap-1'>
            ...
            <span className="font-bold text-[14px]">{props?.item?.brand?.length > 20 ? `${props?.item?.brand?.substr(0, 20)}...` : props?.item?.brand}</span>
          </div>
          <h4 className="text-[14px] line-clamp-1 flex items-center gap-1">
            <Link to={`/product/${props?.item?.productId}`} className="link transition-all" onClick={context.toggleCartPanel(false)}>
              {props?.item?.productTitle?.length > 32 ? `${props?.item?.productTitle?.substr(0, 32)}...` : props?.item?.productTitle}
            </Link>
          </h4>
